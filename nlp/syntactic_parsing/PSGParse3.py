...
class PSG:
    ...
    rule_re = re.compile(
        r"(?P<num>[0-9]+(\.[0-9]*)?)\t(?P<lhssymbol>[a-zA-Z][-a-zA-Z0-9]*)\t(?P<rhssymbols>[a-zA-Z][-a-zA-Z0-9]*(\s+[a-zA-Z][-a-zA-Z0-9]*)*)")

    def __init__(self, filename, is_prob=False):
    ...
    def load(self, filename):
        ...
        while line:
            ...
            if res:
                prob = float(1 if not self.is_prob else res.group('num'))
                if prob == 0:
                    log_prob = 0
                else:
                ...
                ruletuple = (lhs, rhs, log_prob)  # put a log prob of 0 in rule tuple
        ...
        for rule in rules:
            lhs = rule[0]
            rhs = rule[1]
            log_prob = rule[2]  # include the log probability as part of the ruleset
            ...
            rule = (lhs, rhs, log_prob)  # include the log probability while creating hashes for rhs
            if rule not in res:
                res.append(rule)
