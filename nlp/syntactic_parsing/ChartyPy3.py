    ...
    for i in range(len(chart)):
        if not isActive(chart[i]):
            if chart[i][0] == 0 and chart[i][1] == inputlength:  # got spanning edge
                ...
                tree_str = struct2Str(chart[i], chart, grammar)  # call for generating parse tree representation
                if grammar.is_prob:
                    tree_str = tree_str + f"    with a log probability of {chart[i][6]}"
                parses.append(tree_str)
    return parses
...
def struct2Str(edge, chart, grammar):
    ...
    """
    tmpstr = f"{grammar.id2s(edge[3])}["
    dependencies = edge[5]
    for dep in dependencies:
        tmpstr = tmpstr + f" {struct2Str(chart[dep], chart, grammar)}"  # recursively call the method for
        # the child inactive edges which constitutes this edge.

    if len(dependencies):
        tmpstr = tmpstr + f" ]"  # if there were dependencies and the recursive calls from children have returned,
        # then just append a ]
    else:  # otherwise this denotes a non terminal and therefore, the non terminal has to be included.
        tmpstr = tmpstr + f" {grammar.idl2s(edge[4])[0]} ]"
    return tmpstr
...
def ruleInvocation(lststart, chart, inputlength, grammar):
    ...
    for i in range(lststart, len(chart)):
        if chart[i][2] >= len(chart[i][4]):  # only inactive edge
            (start, end, index, lhs, rhs, consumed, log_prob) = chart[i]
            for k in grammar.rhshash.get(lhs, ()):
                if len(k[1]) > inputlength - start:
                    continue
                newedge = (start, end, 1, k[0], k[1], (i,), log_prob + k[2])  # add the log prob here
                if newedge in chart:
                    continue
                ...
                if isInactive(chart[k]):
                    if match(aedge, chart[k]):
                        newedge = (aedge[0], chart[k][1], aedge[2] + 1,
                                   aedge[3], aedge[4], tuple(list(aedge[5]) + [k]), log_prob + chart[k][6])  # add the
                        # log prob here
                        # moving the dot to 1 ahead
...
def parse(inp, grammar):
    ...
    for i in range(inputlength):  # bottom-up parsing
        # import ipdb;ipdb.set_trace()
        # initialize with input token
        rules = grammar.rhshash.get(grammar.symb2id[inp[i]], (("", ())))
        for rule in rules:
            if rule[0]:
                chart.append((i, i + 1, 1, rule[0], rule[1], (), rule[2]))
        if DEBUG:
...
def printParses(parses):
    """Prints the parse trees generated"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--prob", dest="prob", default='0',
                        help="run the Earley algorithm probabilistically")
    args = parser.parse_args()

    if args:
        DEBUG = args.DEBUG
        PROB = args.prob.lower() in ['1', 'true']
        # QTREE = args.latex
        try:
            mygrammar = PSGParse3.PSG(args.grammar, PROB)  # initialization of the grammar
        except IOError:
